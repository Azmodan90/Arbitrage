...
class PairArbitrageStrategy:
    ...
    async def check_opportunity(self, symbol):
        logging.info(f"{self.pair_name} - Sprawdzam okazje arbitrażowe dla symbolu: {symbol}")
        ...
        for key, result in zip(tasks.keys(), results):
            if isinstance(result, Exception):
                logging.error(f"{self.pair_name} - Błąd pobierania danych dla {key}: {result}")
            elif result:
                price = result.get('last')
                if price is None:
                    logging.warning(f"{self.pair_name} - Dla {key} pobrana cena jest None, pomijam {symbol}")
                else:
                    tickers[key] = price
        ...
        if profit1 >= CONFIG["ARBITRAGE_THRESHOLD"]:
            logging.info(f"{self.pair_name} - Okazja arbitrażowa dla {symbol}: Kupno na {self.exchange1.__class__.__name__} (cena: {tickers['ex1']}, efektywna: {effective_buy_ex1:.4f}), sprzedaż na {self.exchange2.__class__.__name__} (cena: {tickers['ex2']}, efektywna: {effective_sell_ex2:.4f}), zysk: {profit1:.2f}%")
        if profit2 >= CONFIG["ARBITRAGE_THRESHOLD"]:
            logging.info(f"{self.pair_name} - Okazja arbitrażowa dla {symbol}: Kupno na {self.exchange2.__class__.__name__} (cena: {tickers['ex2']}, efektywna: {effective_buy_ex2:.4f}), sprzedaż na {self.exchange1.__class__.__name__} (cena: {tickers['ex1']}, efektywna: {effective_sell_ex1:.4f}), zysk: {profit2:.2f}%")

    async def run(self):
        logging.info(f"{self.pair_name} - Uruchamiam strategię arbitrażu dla {len(self.assets)} aktywów.")
        while True:
