import json
import logging
from exchanges.binance import BinanceExchange
from exchanges.kucoin import KucoinExchange
from exchanges.bitget import BitgetExchange
from exchanges.bitstamp import BitstampExchange
from config import CONFIG

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

def get_markets_dict(exchange_instance, allowed_quotes):
    try:
        logging.info(f"Ładowanie rynków dla: {exchange_instance.__class__.__name__}")
        markets = exchange_instance.exchange.load_markets()
        result = {}
        for symbol in markets:
            if "/" in symbol:
                base, quote = symbol.split("/")
                if quote in allowed_quotes:
                    if base not in result:
                        result[base] = symbol
        return result
    except Exception as e:
        logging.error(f"Błąd przy ładowaniu rynków dla {exchange_instance.__class__.__name__}: {e}")
        return {}

def get_common_assets_for_pair_by_quote(name1, exchange1, name2, exchange2, allowed_quotes):
    common = {}
    for quote in allowed_quotes:
        markets1 = get_markets_dict(exchange1, allowed_quotes=[quote])
        markets2 = get_markets_dict(exchange2, allowed_quotes=[quote])
        common_bases = set(markets1.keys()).intersection(set(markets2.keys()))
        if common_bases:
            common[quote] = { base: {name1: markets1[base], name2: markets2[base]} for base in common_bases }
    logging.info(f"Wspólne aktywa dla {name1} i {name2} (quotes={allowed_quotes}): " +
                 f"{sum(len(v) for v in common.values())} znalezione")
    return common

def save_common_assets(common_assets, filename="common_assets.json"):
    try:
        with open(filename, "w", encoding="utf-8") as f:
            json.dump(common_assets, f, indent=4)
        logging.info(f"Lista wspólnych aktywów zapisana do pliku: {filename}")
    except Exception as e:
        logging.error(f"Błąd przy zapisywaniu do pliku {filename}: {e}")

def should_remove(asset, remove_list):
    for r in remove_list:
        if asset == r or asset.startswith(r + "/"):
            return True
    return False

def modify_common_assets(common_assets, remove_file="assets_to_remove.json", add_file="assets_to_add.json"):
    try:
        with open(remove_file, "r", encoding="utf-8") as f:
            assets_to_remove = json.load(f)
        logging.info(f"Wczytano dane do usunięcia z {remove_file}.")
    except Exception as e:
        assets_to_remove = {}
        logging.warning(f"Nie udało się wczytać {remove_file}: {e}")

    try:
        with open(add_file, "r", encoding="utf-8") as f:
            assets_to_add = json.load(f)
        logging.info(f"Wczytano dane do dodania z {add_file}.")
    except Exception as e:
        assets_to_add = {}
        logging.warning(f"Nie udało się wczytać {add_file}: {e}")

    # Zakładamy, że struktura common_assets jest teraz: { "exchange1-exchange2": { "QUOTE": { base: { ... } } } }
    for config_key in list(common_assets.keys()):
        for quote in common_assets[config_key]:
            # Usuwanie
            if config_key in assets_to_remove:
                remove_list = assets_to_remove[config_key]
                before = len(common_assets[config_key][quote])
                common_assets[config_key][quote] = {base: mapping for base, mapping in common_assets[config_key][quote].items()
                                                     if not should_remove(base, remove_list)}
                after = len(common_assets[config_key][quote])
                logging.info(f"Konfiguracja {config_key} [{quote}]: usunięto {before - after} aktywów.")
            # Dodawanie
            if config_key in assets_to_add:
                add_entries = assets_to_add[config_key]
                if quote not in common_assets[config_key]:
                    common_assets[config_key][quote] = {}
                for entry in add_entries:
                    normalized = entry.get("normalized")
                    # Zakładamy, że wpis dotyczy konkretnego quote – możesz dodać w wpisie pole "quote", żeby filtrować
                    entry_quote = entry.get("quote")
                    if entry_quote and entry_quote == quote and normalized and normalized not in common_assets[config_key][quote]:
                        common_assets[config_key][quote][normalized] = {
                            config_key.split("-")[0]: entry.get("source"),
                            config_key.split("-")[1]: entry.get("dest")
                        }
                        logging.info(f"Konfiguracja {config_key} [{quote}]: dodano aktywo {entry}.")
    return common_assets

def main():
    logging.info("Rozpoczynam tworzenie listy wspólnych aktywów (porównanie wyłącznie po symbolu)")
    binance = BinanceExchange()
    kucoin = KucoinExchange()
    bitget = BitgetExchange()
    bitstamp = BitstampExchange()

    exchanges = {
        "binance": binance,
        "kucoin": kucoin,
        "bitget": bitget,
        "bitstamp": bitstamp
    }

    common_assets = {}
    names = list(exchanges.keys())
    allowed_quotes = CONFIG.get("ALLOWED_QUOTES", ["USDT"])
    # Dla każdej pary giełd budujemy mapowanie osobno dla każdego quote
    for i in range(len(names)):
        for j in range(i + 1, len(names)):
            name1 = names[i]
            name2 = names[j]
            logging.info(f"Porównuję aktywa dla pary: {name1} - {name2}")
            mapping = get_common_assets_for_pair_by_quote(name1, exchanges[name1], name2, exchanges[name2], allowed_quotes)
            common_assets[f"{name1}-{name2}"] = mapping
    common_assets = modify_common_assets(common_assets)
    save_common_assets(common_assets)
    for pair, assets in common_assets.items():
        total = sum(len(v) for v in assets.values())
        logging.info(f"Para {pair} ma {total} wspólnych aktywów.")

if __name__ == '__main__':
    main()
