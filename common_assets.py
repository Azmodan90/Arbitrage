...
# Pozwalamy na zagnieżdżanie pętli zdarzeń (przydatne w niektórych środowiskach)
nest_asyncio.apply()

logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')

async def get_markets_dict(exchange_instance, allowed_quotes=["USDT", "EUR", "USDC"]):
    """
    Pobiera rynki z danej giełdy i zwraca słownik:
    ...
    try:
        ...
        # Jeśli wynik jest coroutine, czekamy na jego wykonanie:
...
def modify_common_assets(common_assets, remove_file="assets_to_remove.json", add_file="assets_to_add.json"):
    ...
    for config_key in list(common_assets.keys()):
        ...
        # Dodawanie wpisów – dodajemy tylko, jeśli nie ma danego tokena
...
async def main_async():
    ...
    for ex in exchanges.values():
        close_method = getattr(ex, "close", None)
