...
# Używamy loggera "app" do globalnych komunikatów
...
async def get_markets_dict(exchange_instance, allowed_quotes=None):
    ...
    try:
        app_logger.info(f"Ładowanie rynków dla: {exchange_instance.__class__.__name__}")
        ...
        app_logger.error(f"Błąd przy ładowaniu rynków dla {exchange_instance.__class__.__name__}: {e}")
        return {}
...
async def get_common_assets_for_pair(name1, exchange1, name2, exchange2, allowed_quotes=None):
    ...
    app_logger.info(f"Wspólne aktywa dla {name1} i {name2} (quotes={allowed_quotes}): {len(common)} znalezione")
    return common
...
def save_common_assets(common_assets, filename="common_assets.json"):
    try:
        ...
        app_logger.info(f"Lista wspólnych aktywów zapisana do pliku: {filename}")
    except Exception as e:
        app_logger.error(f"Błąd przy zapisywaniu do pliku {filename}: {e}")
...
def modify_common_assets(common_assets, remove_file="assets_to_remove.json", add_file="assets_to_add.json"):
    try:
        ...
        app_logger.info(f"Wczytano dane do usunięcia z {remove_file}.")
    except Exception as e:
        assets_to_remove = {}
        app_logger.warning(f"Nie udało się wczytać {remove_file}: {e}")
    try:
        with open(add_file, "r", encoding="utf-8") as f:
            assets_to_add = json.load(f)
        app_logger.info(f"Wczytano dane do dodania z {add_file}.")
    except Exception as e:
        assets_to_add = {}
        app_logger.warning(f"Nie udało się wczytać {add_file}: {e}")
    for config_key in list(common_assets.keys()):
        if config_key in assets_to_remove:
            ...
            app_logger.info(f"Konfiguracja {config_key}: usunięto {before - after} aktywów.")
        if config_key in assets_to_add:
            ...
            for entry in add_entries:
                ...
                if normalized and normalized not in common_assets[config_key]:
                    ...
                    app_logger.info(f"Konfiguracja {config_key}: dodano aktywo {entry}.")
    return common_assets

async def main():
    app_logger.info("Rozpoczynam tworzenie listy wspólnych aktywów (porównanie po symbolu oraz quote)")
    ...
    try:
        for i in range(len(names)):
            for j in range(i + 1, len(names)):
                name1 = names[i]
                name2 = names[j]
                app_logger.info(f"Porównuję aktywa dla pary: {name1} - {name2}")
        ...
        for pair, assets in common_assets_dict.items():
            app_logger.info(f"Para {pair} ma {len(assets)} wspólnych aktywów.")
    finally:
        await binance.close()
        await kucoin.close()
        await bitget.close()
