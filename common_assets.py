...
logger = logging.getLogger("common_assets")
if not logger.hasHandlers():
    handler = RotatingFileHandler("common_assets.log", mode="a", maxBytes=5*1024*1024, backupCount=5, encoding="utf-8")
    formatter = logging.Formatter('%(asctime)s - %(levelname)s - %(message)s')
    handler.setFormatter(formatter)
    logger.addHandler(handler)

async def get_markets_dict(exchange_instance, allowed_quotes):
    try:
        logger.info(f"Loading markets for: {exchange_instance.__class__.__name__}")
        ...
        logger.error(f"Error loading markets for {exchange_instance.__class__.__name__}: {e}")
        return {}
...
async def get_common_assets_for_pair(name1, exchange1, name2, exchange2, allowed_quotes):
    ...
    logger.info(f"Common assets for {name1} and {name2} (quotes={allowed_quotes}): {len(common)} found")
    return common
...
def save_common_assets(common_assets, filename="common_assets.json"):
    try:
        ...
        logger.info(f"Common assets list saved to file: {filename}")
    except Exception as e:
